  ...
  public void addCallClicked() {
    ...
    if (activity != null) {
      if (activity.isAutomaticallyMutedByAddCall()) {
        // Since clicking add call button brings user to MainActivity and coming back refreshes
        // mute state, add call button should only be clicked once during InCallActivity shows.
        // Otherwise, we set previous mute state wrong.
        return;
      }
      // Automatically mute the current call
      activity.setAutomaticallyMutedByAddCall(true /* automatically mute */,
          AudioModeProvider.getInstance().getAudioState().isMuted() /* current mute state */);
